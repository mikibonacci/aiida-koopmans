"""
Calculations provided by aiida_koopmans.
...
class KcwCalculation(CalcJob):
    ...
    _default_namelists = ['CONTROL','SCREEN'] # 'PLOT']

    # Keywords that cannot be set by the user but will be set by the plugin
    _blocked_keywords = [('CONTROL', 'outdir', _OUTPUT_SUBFOLDER), ('CONTROL', 'prefix', _PREFIX)]
    ...
    def prepare_for_submission(self, folder):  # pylint: disable=too-many-branches,too-many-statements
        ...
        codeinfo.cmdline_params = settings.pop('CMDLINE', ["-in", self.inputs.metadata.options.input_filename])
        
        # We cannot pass the stdin, as we do not support '<' 'stdin_name' notation;
        # we need the '-in' 'stdin_name' notation. see aiida.common.datastructures
        # so, we do not set the following parameters, and we rely on the cmdline_params.
        # codeinfo.stdin_name = self.inputs.metadata.options.input_filename
        
        # to have this: code.x ... > stdout_name 2>&1, we do the following line. see aiida.common.datastructures
