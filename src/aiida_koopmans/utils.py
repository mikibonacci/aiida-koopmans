...
from aiida_wannier90_workflows.workflows import Wannier90BaseWorkChain
...
from koopmans.processes import CommandLineTool, Process
from koopmans.processes.wjl import WannierJLSplitProcess
...
from aiida.orm import Code, Computer, KpointsData
...
from aiida_koopmans.engine.step_data import AiiDAStepsData
...
WANNIER90_BLOCKED_KEYWORDS = [
    t[1] for t in Wannier90Calculation._BLOCKED_PARAMETER_KEYS
]
ALL_BLOCKED_KEYWORDS = (
    KCW_BLOCKED_KEYWORDS
    + KCP_BLOCKED_KEYWORDS
    + PW_BLOCKED_KEYWORDS
    + WANNIER90_BLOCKED_KEYWORDS
    + PROJWFC_BLOCKED_KEYWORDS
    + [f"celldm({i})" for i in range(1, 7)]
)


def get_builder_from_ase(calculator: Calc, step_data: AiiDAStepsData | None = None):
...
def get_PwBaseWorkChain_from_ase(
    pw_calculator: PWCalculator, step_data: AiiDAStepsData
) -> tuple[ProcessBuilder, AiiDAStepsData]:

    aiida_inputs = step_data.configuration
    calc_params = pw_calculator.parameters
    ...
    for step_uid, val in step_data.steps.items():
        if "-scf" in step_uid and (
            "nscf" in pw_calculator.uid or "bands" in pw_calculator.uid
        ):
    ...
    pw_overrides: dict[str, dict[str, Any]] = {
        "CONTROL": {},
        "SYSTEM": {
    ...
    builder: ProcessBuilder = PwBaseWorkChain.get_builder_from_protocol(
    ...
    if "npools" in aiida_inputs.keys():
        builder.pw.parallelization = orm.Dict(dict={"npool": aiida_inputs["npools"]})
    ...
    if pw_overrides["CONTROL"]["calculation"] in ["scf", "nscf"]:
        ...
        builder.kpoints.set_kpoints(
            calc_params["kpts"].kpts, cartesian=False
        )  # TODO: check cartesian false is correct.
...
def get_Wannier90BaseWorkChain_builder_from_ase(
    w90_calculator: Wannier90Calculator,
    step_data: AiiDAStepsData,
    structure: orm.StructureData,
    projection_type: WannierProjectionType,
    projections: orm.List | None = None,
    kpoint_grid: orm.KpointsData | None = None,
    kpoint_path: orm.KpointsData | None = None,
    shift_energy_windows: bool = False,
):

    builder: ProcessBuilder = Wannier90BaseWorkChain.get_builder_from_protocol(
        code=orm.load_code(step_data.configuration["wannier90_code"]),
        structure=structure,
        protocol="moderate",
    ...
    builder.shift_energy_windows = shift_energy_windows
    builder.wannier90.kpoints = kpoint_grid
    builder.wannier90.bands_kpoints = kpoint_path

    # Build the local input folder out of all of the linked files
    local_input_folder = FolderData()
    for filename, file_info in w90_calculator.linked_files.items():
        # We can't add SinglefileData objects to a FolderData, so we are forced to read
        # the content of the file and feed this to the FolderData.
        content = file_info[0].read_text()
        local_input_folder.put_object_from_filelike(
            StringIO(content), filename.replace(w90_calculator.prefix, "aiida")
        )

    builder.wannier90.local_input_folder = local_input_folder

    return builder, builder.wannier90, step_data


def set_aiida_kpoint_path_from_ase(aiida_path: KpointsData, ase_path: BandPath):
    k_coords = []
    k_labels = []

    k_path = ase_path.kpts
    special_k = ase_path.special_points
    k_linear, special_k_coords, special_k_labels = ase_path.get_linear_kpoint_axis()
    t = 0
    ...
    aiida_path.set_kpoints(k_path, labels=k_labels, cartesian=False)


def convert_projections_to_aiida(projections: list[dict[str, Any]]) -> orm.List:
    converted_projs: list[str] = []
    for proj in projections:
        ...
        if proj["fractional_site"]:
            ...
            raise ValueError(f"No site information found in the projection: {proj}")
        orbital = proj["ang_mtm"]
        converted_proj = position + ":" + orbital
        converted_projs.append(converted_proj)

    return orm.List(list=converted_projs)


def get_Wannier90BandsWorkChain_builder_from_ase(
    w90_calculator: Wannier90Calculator,
    step_data: AiiDAStepsData,
    structure: orm.StructureData,
    projection_type: WannierProjectionType,
    ...
    kpoint_path: orm.KpointsData | None = None,
    shift_energy_windows: bool = False,
) -> tuple[ProcessBuilder, Wannier90Calculation, AiiDAStepsData]:
    ...
    if all(w90_calculator.atoms.pbc):
        w90_wchain = Wannier90BandsWorkChain
    else:
        raise NotImplementedError(
            "Wannier90BandsWorkChain is not implemented for non-periodic systems."
        )
    ...
    # Fetch the NSCF step
    nscf = find_nscf_step(step_data)

    # kpoints
    builder.bands_kpoints = kpoint_path
    builder.wannier90.wannier90.kpoints = kpoint_grid

    # Resources
    builder.pw2wannier90.pw2wannier90.metadata = aiida_inputs.get(
        "metadata_pw2wannier90", aiida_inputs["metadata"]
    )
    builder.pw2wannier90.pw2wannier90.parent_folder = nscf.outputs.remote_folder
    builder.wannier90.shift_energy_windows = shift_energy_windows
    ...
    if nscf.inputs.pw.parameters.get_dict()["SYSTEM"]["nspin"] > 1:
        params_pw2wannier90["inputpp"][
            "spin_component"
        ] = builder.wannier90.wannier90.parameters.get_dict().get("spin", "up")
    builder.pw2wannier90.pw2wannier90.parameters = orm.Dict(dict=params_pw2wannier90)

    return builder, builder.wannier90.wannier90, step_data


def get_Wannier90WorkChain_builder_from_ase():
    raise NotImplementedError()


def find_nscf_step(step_data: AiiDAStepsData):
    nscf = None
...
def get_w90_builder_from_ase(
    w90_calculator: Wannier90Calculator, step_data: AiiDAStepsData
):
    """Get a Wannier90Workchain, a Wannier90BaseWorkChain, or a Wannier90BandsWorkChain builder.

    Use the base workchain if we don't want to do preprocessing and pw2wannier90; use the bands workchain
    if we do.
    """
    ...
    projection_type = (
        ...
        if w90_calculator.parameters.projections
        else WannierProjectionType.ATOMIC_PROJECTORS_QE
    )

    # Structure
    structure_pk = step_data.structure
    if structure_pk is None:
        ase_atoms = Atoms.fromdict(w90_calculator.atoms.todict())
        structure = orm.StructureData(ase=ase_atoms)
    else:
    ...
    projections = None
    if w90_calculator.parameters.projections:
        projections = convert_projections_to_aiida(
            w90_calculator.parameters.projections
        )
    ...
    nscf = find_nscf_step(step_data)
    kpoint_grid = orm.KpointsData()

    # kpoint_grid = nscf.inputs.kpoints
    # kpoint_grid.set_kpoints_mesh(*nscf.inputs.kpoints.get_kpoints_mesh())
    kpoint_grid.set_cell_from_structure(nscf.inputs.pw.structure)
    grid = np.round(nscf.outputs.output_band.get_array("kpoints"), decimals=10)
    grid[grid == -0.0] = 0.0
    kpoint_grid.set_kpoints(grid, cartesian=False)
    ...
    kpoint_path = orm.KpointsData()
    kpoint_path.set_cell_from_structure(nscf.inputs.pw.structure)
    ase_kpoint_path = None
    if hasattr(w90_calculator.parameters, "kpoint_path"):
        ase_kpoint_path = w90_calculator.parameters.kpoint_path
    if all([p for p in nscf.inputs.pw.structure.pbc]) and ase_kpoint_path:
        set_aiida_kpoint_path_from_ase(kpoint_path, ase_kpoint_path)

    # Construct the builder
    reading_from = list(
        {f[0].parent_process for f in w90_calculator.linked_files.values()}
    )
    kwargs = {}
    if all([isinstance(c, PW2WannierCalculator) for c in reading_from]):
        # If the files come from a P2WannierCalculator, we want to run a Wannier90BandsWorkChain
        get_builder = get_Wannier90BandsWorkChain_builder_from_ase
        if all(w90_calculator.atoms.pbc):
            kwargs["kpoint_grid"] = kpoint_grid
            kwargs["kpoint_path"] = kpoint_path
    else:
        get_builder = get_Wannier90BaseWorkChain_builder_from_ase
        kwargs["kpoint_grid"] = kpoint_grid
        kwargs["kpoint_path"] = kpoint_path
    # Shift energy windows is hard-coded to False for the moment
    builder, wannier90, step_data = get_builder(
        w90_calculator,
        step_data=step_data,
        structure=structure,
        projection_type=projection_type,
    ...
    # Parameters
    params = wannier90.parameters.get_dict()
    ...
    wannier90.parameters = orm.Dict(dict=params)
    ...
    wannier90.metadata = step_data.configuration["metadata"]
    wannier90.metadata.options.resources = default_w90_metadata_options_resources

    # Make sure the wannier90 input file is also retrieved (sometimes required by subsequent WannierJL calculations)
    wannier90.metadata.options.additional_retrieve_list = [
        "aiida.win",
        "aiida.chk",
        "aiida.mmn",
        "aiida.eig",
...
def get_projwfc_builder_from_ase(projwfc_calculator, step_data=None):
    ...
    parent_calculators = [
        f[0].parent_process.uid
        for f in projwfc_calculator.linked_files.values()
        if f[0] is not None
    ]
    ...
    if len(set(parent_calculators)) > 1:
        ...
        remote_folder = step_data.steps[parent_calculators[0]].remote_folder
        if remote_folder is not None:
            builder.parent_folder = orm.load_node(remote_folder)
        else:
            raise ValueError(f"No remote folder found for {parent_calculators[0]}.")
    else:
        raise ValueError("No parent calculator found for the projwfc.x calculation.")

    return builder, step_data


def get_kcw_builder_from_ase(kcw_calculator, step_data: AiiDAStepsData):
    ...
    unique_manifold = kcw_calculator.parameters.get("l_unique_manifold", False)
    ...
    for filename, (file, _, _, _) in kcw_calculator.linked_files.items():
        try:
            saved_files = step_data.steps[file.parent_process.uid].input_files
        except:
            raise ValueError()
        if filename in saved_files:
            if "wannier90_u.mat" in filename:
                if unique_manifold:
                    wann_u_mat = orm.load_node(saved_files[filename])
                else:
                    wann_occ_u_mat = orm.load_node(saved_files[filename])
            elif "wannier90_emp_u.mat" in filename:
                wann_emp_u_mat = orm.load_node(saved_files[filename])
            elif "wannier90_centres.xyz" in filename:
                if unique_manifold:
                    wann_centres_xyz = orm.load_node(saved_files[filename])
                else:
                    wann_occ_centres_xyz = orm.load_node(saved_files[filename])
            elif "wannier90_emp_centres.xyz" in filename:
                wann_emp_centres_xyz = orm.load_node(saved_files[filename])
            elif "wannier90_u_dis.mat" in filename:
                wann_u_dis_mat = orm.load_node(saved_files[filename])
            elif "wannier90_emp_u_dis.mat" in filename:
                wann_emp_u_dis_mat = orm.load_node(saved_files[filename])
            elif "file_alpharef.txt" in filename:
                alpha = orm.load_node(saved_files[filename])
            else:
                raise ValueError(
                    f"Unknown file {filename} in the kcw_calculator linked files."
                )
        else:
            warn(f"File {filename} not found in the step data input files. ")
    ...
    for step_uid, val in step_data.steps.items():
        ...
        if "kcw_wannier" in step_uid and val.workchain is not None:
        ...
        # if "merge_occ_wannier_u" in step_uid and wann_u_mat is None:
        #     wann_u_mat = orm.load_node(val.input_files['wannier90_u.mat'])
        # if "merge_occ_wannier_centers" in step_uid and wann_centres_xyz is None:
        ...
        # if "merge_emp_wannier_centers" in step_uid and wann_emp_centres_xyz is None:
        #     wann_emp_centres_xyz = orm.load_node(val.input_files['wannier90_centres.xyz'])
        # if "merge_emp_wannier_u_dis" in step_uid and wann_emp_u_dis_mat is None:
    ...
    if control_dict.get("read_unitary_matrix", read_unitary_matrix):
        if unique_manifold:
            assert wann_u_mat is not None, "wann_u_mat has not been provided"
            builder.wann_u_mat = wann_u_mat
            assert (
                wann_centres_xyz is not None
            ), "wann_centres_xyz has not been provided"
            builder.wann_centres_xyz = wann_centres_xyz
            assert wann_u_dis_mat is not None, "wann_u_dis_mat has not been provided"
            builder.wann_u_dis_mat = wann_u_dis_mat
        else:
            builder.wann_occ_u_mat = wann_occ_u_mat
            builder.wann_occ_centres_xyz = wann_occ_centres_xyz
            builder.wann_emp_u_mat = wann_emp_u_mat
            builder.wann_emp_centres_xyz = wann_emp_centres_xyz
            builder.wann_emp_u_dis_mat = wann_emp_u_dis_mat
...
def get_kcp_builder_from_ase(kcp_calculator, step_data: AiiDAStepsData):
    ...
    for step_uid, val in step_data.steps.items():
        ...
        if (
            "dft_init_nspin2" in kcp_calculator.uid
            and not "dummy" in kcp_calculator.uid
        ):
    ...
    if hasattr(kcp_calculator, "linked_files"):
        if (
            hasattr(kcp_calculator, "alphas")
            and not "input_files"
            in kcp_calculator.engine.step_data.steps[str(kcp_calculator.directory)]
        ):
            ...
            if "input_files" in step:
                ...
                file_alpharef_empty = orm.load_node(
                    step["input_files"]["file_alpharef_empty.txt"]
                )

        parent_directory = None

        additional_files_K0001 = (
            {}
        )  # each key is the pk of a process, and the value is a list of file names to be used in the kcp calcjob.
        additional_files_wann2kcp = {}
        for file_names in kcp_calculator.linked_files.keys():
            if (
                "TMP-CP" in file_names
                and file_names.split("/")[-1] == "TMP-CP"
                or "save" in file_names.split("/")[-1]
            ):  # not the case when pointing only to given files.
                parent_directory = str(
                    kcp_calculator.linked_files[file_names][0].parent_process.directory
                )
                step = step_data.steps[parent_directory]
                if step.workchain is not None:
                    parent_folder = orm.load_node(step.workchain).outputs.remote_folder
        for file_names in kcp_calculator.linked_files.keys():
            second_parent_directory = str(
                kcp_calculator.linked_files[file_names][0].parent_process.directory
            )
            ...
            if "pz_print" in second_parent_directory:
                if parent_directory and parent_directory != second_parent_directory:
                    ...
                    additional_files_K0001[str(step.workchain)].append(
                        file_names.split("/")[-1]
                    )
            elif (
                "convert" in second_parent_directory
                or "merge_wavefunctions" in second_parent_directory
            ):
                if parent_directory and parent_directory != second_parent_directory:
                    step = step_data.steps[second_parent_directory]
                    ...
                    additional_files_wann2kcp[str(step.workchain)].append(
                        (
                            str(file_names).split("/")[-1],
                            str(kcp_calculator.linked_files[file_names][0].name).split(
                                "/"
                            )[-1],
                        )
                    )
    ...
    if files_to_copy:
        builder.spin2_files = orm.List(
            list=[(k, v) for k, v in files_to_copy.items() if v is not None]
        )
...
def get_wann2kcp_builder_from_ase(wann2kcp_calculator, step_data: AiiDAStepsData):
    ...
    parent_wannier = orm.load_node(
        step_data.steps[
            str(
                wann2kcp_calculator.linked_files["wannier90.chk"][
                    0
                ].parent_process.directory
            )
        ]["workchain"]
    )
    additional_files = {}
    ...
    for file_names in wann2kcp_calculator.linked_files.keys():
        if "TMP" in file_names:
            parent_directory = str(
                wann2kcp_calculator.linked_files[file_names][0].parent_process.directory
            )
            step = step_data.steps[parent_directory]
            if "workchain" in step:
                parent_folder = orm.load_node(step["workchain"]).outputs.remote_folder
        else:
            parent_remote_wannier = (
                parent_wannier.outputs.wannier90_pp.remote_folder.uuid
                if "nnkp" in file_names
                else parent_wannier.outputs.wannier90.remote_folder.uuid
            )
            if not parent_remote_wannier in additional_files.keys():
                additional_files[parent_remote_wannier] = []
            additional_files[parent_remote_wannier].append(
                file_names.replace("wannier90", "aiida")
            )
...
def prepare_shell_job(process: CommandLineTool, step_data: None):
    ...
    if hasattr(process, "linked_files"):
        for j, file_name in enumerate(process.linked_files.keys()):
            remote = orm.load_node(
                step_data.steps[
                    str(process.linked_files[file_name].parent_process.directory)
                ]["remote_folder"]
            ).get_remote_path()
