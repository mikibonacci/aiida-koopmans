...
class Wann2kcpParser(BaseParser):
    """``Parser`` implementation for the ``KcpCalculation`` calculation job class.

    For now it just checks if there is the `JOB DONE` string at the end, and some other common functionalities (BaseParser).
    """
    
    def parse(self, **kwargs):
        """Parse the retrieved files from a ``KcpCalculation`` into output nodes."""
    ...
    def _parse_xml(self, retrieved_temporary_folder):
        ...
        xml_filepath = Path(retrieved_temporary_folder) / "data-file.xml"

        if not xml_filepath.exists():
            self.exit_code_xml = self.exit_codes.ERROR_MISSING_XML_FILE
