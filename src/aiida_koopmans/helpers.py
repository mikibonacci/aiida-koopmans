...
def get_wannier90bandsworkchain_builder_from_ase(w90_calculator):
    ...
    (
        ...
        special_k_coords,
        special_k_labels,
    ) = w90_calculator.parameters.kpoint_path.get_linear_kpoint_axis()
    t = 0
    ...
    for proj in w90_calculator.todict()["_parameters"]["projections"]:
        ...
        if "fractional_site" in proj.keys():
            position = "f=" + str(proj["fractional_site"]).replace("[", "").replace(
                "]", ""
            ).replace(" ", "")
    ...
    params.pop(
        "auto_projections", None
    )  # Uncomment this if you want analytic atomic projections
    ...
    builder.wannier90.wannier90.metadata = aiida_inputs.get(
        "metadata_w90", default_w90_metadata
    )
...
def aiida_check_code_is_installed_trigger(check_code_is_installed):
    ...
    def wrapper_aiida_trigger(self):
        if self.parameters.mode == "ase":
            return check_code_is_installed(
                self,
            )
        else:
            return
...
def aiida_fetch_linked_files_trigger(_fetch_linked_files):
    ...
    def wrapper_aiida_trigger(self):
        if self.parameters.mode == "ase":
                ...
                if hasattr(src_calc, "wchain"):
                    ...
                    if "wannier90" in dest_filename:
                        if "emp" in dest_filename:
                            self.wannier90_files["emp"] = produce_wannier90_files(
                                src_calc, "emp"
                            )
                        else:
                            self.wannier90_files["occ"] = produce_wannier90_files(
                                src_calc, "occ"
                            )
                    else:
            ...
            if self.wannier90_files == {}:
                delattr(self, "wannier90_files")
    ...
    def wrapper_aiida_trigger(calc, relpath):
        ...
        if calc.parameters.mode == "ase":
            ...
            if calc.ext_out == ".wout":
                ...
                filename = relpath.name.replace(
                    "wann", "aiida"
                )  # middle hard coding for wann stuff.
            else:
                inner_remote_folder = None
                filename = "aiida" + calc.ext_out
            return get_output_content(
                calc, filename, mode="r", inner_remote_folder=inner_remote_folder
            )
...
def aiida_write_content_trigger(write_content):
    ...
    def wrapper_aiida_trigger(dst_file, merged_filecontents, actor):
        ...
        if calc.parameters.mode == "ase":
            ...
            if calc.ext_out == ".wout":
                if not hasattr(actor, "singlefiledata"):
                    actor.singlefiledata = {}
...
def aiida_write_alphas_trigger(write_alphas):
    ...
    def wrapper_aiida_trigger(self):
        if self.parameters.mode == "ase":
            ...
            return generate_alpha_singlefiledata(
                self,
            )
    ...
    def wrapper_aiida_trigger(self, calc):
        if calc.prefix in ["wannier90_preproc", "pw2wannier90"]:
            ...
            calc_nscf = [
                ...
                if isinstance(c, calculators.PWCalculator)
                and c.parameters.calculation == "nscf"
            ][-1]
