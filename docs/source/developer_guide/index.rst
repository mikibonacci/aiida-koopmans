...
``aiida-koopmans`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-koopmans`` repository (preferably using ``aiida-koopmans`` as the project name)

The documentation is now available at `aiida-koopmans.readthedocs.io <http://aiida-koopmans.readthedocs.io/>`_.
